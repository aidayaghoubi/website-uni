...
const HeaderControl = (props) => {
    ...
    const inputOncanegHandler = () => {
        ...
        const searchItem = availbalePCTX.filter( (item) => {return  item.P_name.toLowerCase().includes(enteredName.toLowerCase())})
        return searchItem
    ...
    return (
        ...
        <div className={classes['header-control']}>
            <ul className={classes.nav}>
                ...
                {searchIsClicked ? <li className={classes.inputCntrol}>
                    <input ref={searchInputRef} onChange={inputOncanegHandler} onBlur={onBlurInputHandler}></input>
        ...
        { searchIsClicked ? <SearchCart data={dataShouldShow}/> : ''}
        </Fragment>
    )
}
